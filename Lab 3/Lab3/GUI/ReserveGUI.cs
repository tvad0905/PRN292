...
namespace Lab3.GUI
{
    public partial class ReserveGUI : Form
    {
        ...
        private void View()
        {
            ...
            lbNumberOfReservedBooks.Text = (dataGridView1.Rows.Count -1 ).ToString();
        }

        private void btReserve_Click(object sender, EventArgs e)
        {
            Reservation rs = new Reservation(int.Parse(txtMemberCode.Text), int.Parse(txtBookNumber.Text), DateTime.ParseExact(txtDate.Text, "dd/MM/yyyy", null));
            new ReserveDAO().Update(rs);
